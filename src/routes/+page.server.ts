...
export async function load() {
    ...
    const projects: ProjectDocumentData[] = await ProjectSchema.aggregate([{ $sample: { size: 5 } }, { $match: { publish: false }}, { $project: { studentId: 1, title: 1, year: 1, tags: 1, mentorId: 1, shortDesc: 1 }}]) || [];
