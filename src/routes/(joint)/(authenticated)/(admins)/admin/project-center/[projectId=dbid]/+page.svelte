...
<script lang=ts>
    ...
    import { Save, CircleCheckBig, CircleX, ArrowLeft, BookOpen, Trash2, FileText, ExternalLink } from "lucide-svelte";
    ...
    function handlePdfSelection(event: Event) {
        ...
        if (!file) { selectedPdf = null; return; }
        if (file.size > MAX_PDF_SIZE_BYTES) {
            selectedPdf = null;
            (input as HTMLInputElement).value = "";
            toast.error(`File too large. Max: ${bytesToString(MAX_PDF_SIZE_BYTES)}`);
            return;
        }
        selectedPdf = file;
    }
    ...
    async function removePdf() {
        removingPdf = true;
        const res = await fetch(`/manage-projects/${project._id}/report/pdf`, { method: "DELETE" });
        removingPdf = false;
        if (!res.ok) {
            const body = await res.json().catch(() => ({}));
...
<main class="w-full h-full p-4 overflow-y-auto">
    <div class="relative w-full rounded-xl p-8 flex flex-col gap-6">
        <a class="absolute top-0 left-0 bg-blue-500 hover:bg-blue-600 p-2 px-3 m-8 rounded-lg flex gap-2 items-center text-white" href="/admin/project-center">
            <ArrowLeft size={18} />
        ...
        <div class="grid grid-cols-2 gap-4 bg-gray-100 rounded-xl p-6 border border-gray-300">
            ...
            <div>
                ...
                <p class="text-sm text-gray-500">{project.mentor?.organization}</p>
            </div>
            <div>
        ...
        <div class="bg-gray-100 rounded-xl p-6 border border-gray-300 flex flex-col gap-4">
            ...
            <div class="flex items-center gap-3">
                ...
                >
                    {#if reportStatus === SaveStatus.Saving}<Circle3 size={20} />{:else}<Save size={18} />{/if}
        ...
        <!-- PDF Section -->
        <div class="bg-gray-100 rounded-xl p-6 border border-gray-300 flex flex-col gap-4">
            <h2 class="text-xl font-semibold flex items-center gap-2"><FileText size={22} /> PDF Report</h2>
            {#if pdfUrl}
            <div class="flex items-center gap-4">
                <a href={pdfUrl} target="_blank" class="flex items-center gap-2 bg-blue-500 hover:bg-blue-600 text-white rounded-md p-2 px-4">
                ...
                >
                    {#if removingPdf}<Circle3 size={18} />{:else}<Trash2 size={18} />{/if}
            ...
            <div class="flex items-center gap-3">
                <input type="file" accept=".pdf,application/pdf" onchange={handlePdfSelection} class="text-sm" />
                {#if selectedPdf}
                <button
                    class="flex items-center gap-2 bg-blue-500 hover:bg-blue-600 disabled:bg-gray-400 text-white rounded-md p-2 px-4"
                    onclick={uploadPdf}
                ...
                >
                    {#if uploadingPdf}<Circle3 size={18} />{:else}<Save size={18} />{/if}
        ...
        <!-- Danger zone: delete -->
        <div class="bg-red-50 rounded-xl p-6 border border-red-300 flex flex-col gap-4">
            <h2 class="text-xl font-semibold text-red-700 flex items-center gap-2"><Trash2 size={22} /> Delete Project</h2>
            <p class="text-sm text-red-600">This action is irreversible. Type <strong>{DELETE_PHRASE}</strong> to confirm.</p>
            <input
                type="text"
            ...
            >
                {#if deleting}<Circle3 size={18} />{:else}<Trash2 size={18} />{/if}
