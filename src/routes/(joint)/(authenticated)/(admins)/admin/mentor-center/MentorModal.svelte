<script lang="ts">
    ...
    let saveState = $state(Status.Unchanged);
    ...
    $effect(() => {
        ...
        organization;
        phoneNumber;
        if (changed) saveState = Status.Unsaved;
        if (!changed) saveState = Status.Unchanged;
    });
...
<Modal>
    <div class="relative">
        <form
            class="flex flex-col rounded-lg bg-gray-300 p-8 items-start"
            onsubmit={updateInformation}
        >
            <span class="text-2xl font-bold mb-4">Mentor Details</span>

            <div
                class="grid grid-rows-[auto_auto] grid-cols-[auto_auto] gap-4 mb-2"
            >
                <div class="flex flex-col items-start">
                    ...
                    <span
                        ...
                        class="mb-1 text-base rounded-md bg-gray-50 p-2 border-gray-400 border min-w-72 max-w-[16rem]"
                    ></span>
                    <span class="text-gray-600 max-w-[24rem] text-sm"
                        >Ex: Thomas Zhou, Bill Gates (just the name, no titles,
                        etc.)</span
                    >
                </div>
                ...
                <div class="flex flex-col items-start">
                    ...
                    <span
                        bind:textContent={email}
                        contenteditable="plaintext-only"
                        class="mb-1 text-base rounded-md bg-gray-50 p-2 border-gray-400 border min-w-72 max-w-[16rem]"
                    ></span>
                    <span class="text-gray-600 max-w-[24rem] text-sm"
                        >Ex: tzhou@commschool.org</span
                    >
                </div>
                ...
                <div class="flex flex-col items-start">
                    ...
                    <span
                        bind:textContent={organization}
                        contenteditable="plaintext-only"
                        class="mb-1 text-base rounded-md bg-gray-50 p-2 border-gray-400 border min-w-72 max-w-[16rem]"
                    ></span>
                    <span class="text-gray-600 max-w-[24rem] text-sm"
                        >Ex: Brookline Food Pantry, Center of Inclusive
                        Computing at Northeastern University, Columbia
                        University, Kaggle, Commonwealth School</span
                    >
                </div>

                <div class="flex flex-col items-start">
                    <span class="text-lg mb-1">Mentor Phone: </span>
                    <span class="text-gray-600 max-w-[24rem] text-sm mb-1"
                        >*Optional</span
                    >
                    <span
                        bind:textContent={phoneNumber}
                        contenteditable="plaintext-only"
                        class="mb-1 text-base rounded-md bg-gray-50 p-2 border-gray-400 border min-w-72 max-w-[16rem]"
                    ></span>
                    <span class="text-gray-600 max-w-[24rem] text-sm"
                        >Ex: +1 (427)-211-9772, 324-474-1106, 8568224318</span
                    >
                </div>
            </div>
            ...
            <div class="flex gap-2 items-center">
                <button
                    type="submit"
                    class="relative {saveState == Status.Saved ||
                    saveState == Status.Unchanged
                        ? 'bg-gray-400 cursor-not-allowed'
                        : 'bg-blue-500 hover:bg-blue-600'}  rounded-md p-2 text-white inline-flex items-center px-3 gap-2"
                    disabled={saveState == Status.Saved ||
                        saveState == Status.Unchanged}
                >
                    <Save />
                    <span>Save</span>
                    {#if saveState == Status.Unsaved}
                        <div
                            class="absolute bg-red-500 rounded-full -top-2 -right-3 w-6 h-6"
                        >
        ...
        <button class="absolute top-0 right-0 m-4" onclick={onclose}
            ><X /></button
        >
    </div>
</Modal>
