...
export async function load({ url: { searchParams } }) {
    ...
    const inflatedImages = await Promise.all((await ImageSchema.find({}, 'projectId description').lean().sort('-createdAt').limit(items).skip(skip))?.map(stringifyObjectId).map(injectProjects));
