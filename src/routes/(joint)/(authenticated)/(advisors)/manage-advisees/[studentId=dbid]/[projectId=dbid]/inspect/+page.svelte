<script lang="ts">
    ...
    async function changeApproval(project: any, approval: boolean) {
        const res = await fetch(
            `/manage-advisees/${data.studentId}/${data.project._id}/approval`,
            {
                method: "POST",
                body: JSON.stringify({
...
{#if initalPrompt}
    <div
        class="absolute flex items-center justify-center h-full w-full top-0 left-0 bg-[#000000AA]"
    >
        <div
            class="bg-gray-200 p-8 rounded-lg gap-4 flex flex-col items-center max-w-[90vw] sm:max-w-[75vw]"
        >
            <p class="gap-1 text-xl">
                <strong class="text-red-700">IMPORTANT:</strong> Please review everything,
                the button to approve will appear at the bottom once you have confirmed
                that the required elements are present!
            </p>
            <button
                class="bg-gray-400 hover:bg-gray-500 text-white p-2 rounded-md flex items-center gap-2 px-4"
                onclick={() => (initalPrompt = false)}
            >
...
<main class="flex flex-col items-center w-full bg-gray-300">
    <div
        class="grid grid-cols-[1fr_auto_1fr] w-full p-12 items-center justify-center"
    >
        <div class="flex justify-start items-center">
            <button
                class="bg-blue-500 hover:bg-blue-600 text-white p-2 rounded-md flex items-center gap-2 px-4"
        ...
        <h1 class="text-3xl">
            Inspecting {data.project.student.name}'s Project
        </h1>
        <div></div>
    </div>
    <div
        class="inline-flex flex-col p-8 sm:p-16 m-4 sm:m-16 !mt-0 bg-gray-200 rounded-lg border border-solid border-gray-400"
    >
        ...
        <p class="mentor">
            ...
            {data.project.mentor.name}
        </p>
        <p class="mentor">
            <strong>Mentor Info:</strong>
            {data.project.mentor.email}, {data.project.mentor.phoneNumber}
        </p>
        <p class="student">
            <strong>Student:</strong>
            {data.project.student.name}
        </p>
        <p class="created-at">
            <strong>Created:</strong>
            {new Date(data.project.createdAt).toLocaleString()}
        </p>
        <p class="created-at">
        ...
        <div class="pdf-text-grid w-full mb-8">
            <div class="bg-white border border-gray-300 rounded-lg p-4">
                ...
                {#if data.project.pdfUrl}
                    <iframe
                        ...
                        class="w-full h-[28rem] border border-gray-300 rounded"
                    ></iframe>
                {:else}
            ...
            <div
                class="w-full min-h-full bg-white whitespace-pre-wrap break-words p-8 border border-gray-300 rounded-lg flex flex-col"
            >
                <div
                ...
                >
                    ...
                    <button
                        class="bg-gray-500 hover:bg-gray-600 text-white p-2 rounded-md flex items-center gap-2 px-4"
                    ...
                    {:else}
                        ...
                        >
                            <span class="text-4xl w-full text-center"
                                >No text report has been uploaded for this project.</span
                            >
        ...
        <div class="flex justify-start">
            {#if data.project.underReview == false}
                <button
                    class="bg-red-500 hover:bg-red-600 text-lg p-4 px-8 text-white rounded-lg"
                    onclick={() => changeApproval(data.project, false)}
                    >Unapprove Project</button
                >
            {:else if infoGood && reportGood}
                <button
                    class="bg-green-500 hover:bg-green-600 text-lg p-4 px-8 text-white rounded-lg"
                    onclick={() => changeApproval(data.project, true)}
                    >Approve Project</button
                >
            {:else}
                <span class="text-gray-600"
                    >The button to approve will appear once you have confirmed
                    the required elements are present.</span
                >
            {/if}
        </div>
    </div>
