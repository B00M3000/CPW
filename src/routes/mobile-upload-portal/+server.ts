...
export async function POST({ request }) {
    ...
    if (!mobileKeyDoc)
        error(404, {
            message:
                "Mobile key not found! Please try again. (Mobile keys are only valid for a short period after they are generated.)",
        });
    ...
    const { totalSize } = await ImageSchema.aggregate([
        { $match: { ownerId: mobileKey.userId } },
    ...
    if (totalSize + imageFile.size > _USER_IMAGE_SPACE_LIMIT)
        error(400, {
            message:
                "You exceed the alloted amount of space by uploading this image. Please ask website administrators for an expanded quota or manage your assets accordingly.",
        });
