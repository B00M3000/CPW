...
import {
    MobileKeySchema,
    type MobileKeyDocument,
} from "@/server/mongo/schemas/mobileKey";
...
export async function load({ params, locals, url }) {
    ...
    // }
    const user = stringifyObjectId(
        await MobileKeySchema.findById(mobileKeyDoc.userId).lean(),
    );
    const project = stringifyObjectId(
        await MobileKeySchema.findById(mobileKeyDoc.projectId).lean(),
    );
