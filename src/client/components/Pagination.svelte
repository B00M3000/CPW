<script lang=ts>
    ...
    let currentLower = $derived(Math.min((currentPage + 1) * itemsPerPage, maxItems))
</script>
...
<div class="flex items-center justify-between bg-[rgb(236,238,241)] gap-4 w-full rounded-md text-black p-4">
    ...
    <div class="flex items-center h-full gap-3 sm:gap-5">
        <div class="flex items-center gap-1">
            <div>
                <select class="rounded-sm p-1 bg-white" bind:value={currentPage}>
        ...
        <button class="h-full p-2 sm:px-4 bg-gray-300 hover:bg-gray-400 rounded-md" onclick={() => currentPage = Math.max(0, currentPage - 1)}>
            <CaretLeft size={24} /> 
        </button>
        <button class="h-full p-2 sm:px-4 bg-gray-300 hover:bg-gray-400 rounded-md" onclick={() => currentPage = Math.min(totalPages - 1, currentPage + 1)}>
            <CaretRight size={24} />
        </button>
    </div>
</div>
